    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="qr_dia.php" class="nav-link">
                <span class="nav-icon"><i class="fas fa-qrcode"></i></span> QR del DÃ­a
            </a>
        </nav>
        <div class="sidebar-footer">
            ...
            <button class="btn btn-secondary btn-block btn-sm" onclick="logout()">Cerrar SesiÃ³n</button>
        </div>
    </aside>
    ...
    <main class="main-content">
        ...
        <div class="section active" id="section-resumen">
            ...
            <div class="glass-card" style="padding:20px;">
                <h3 style="margin-bottom:12px;"><i class="fas fa-bolt"></i> Acceso RÃ¡pido</h3>
                <div style="display:flex;gap:10px;flex-wrap:wrap;">
                    <a href="qr_dia.php" class="btn btn-primary"><i class="fas fa-qrcode"></i> Generar QR del DÃ­a</a>
        ...
        <div class="section" id="section-clases">
            <div class="glass-card add-form">
                ...
                <div class="form-group"><input type="text" id="claseDesc" class="form-input" placeholder="DescripciÃ³n (opcional)"></div>
        ...
        <div class="section" id="section-horarios">
            <div class="glass-card add-form" id="horarioForm">
                ...
                <div class="form-group">
                    <select id="horarioDia" class="form-input">
                        ...
                        <option value="5">Viernes</option><option value="6">SÃ¡bado</option>
                    </select>
                </div>
            ...
            <div class="glass-card" style="padding:12px;margin-bottom:16px;">
                <div class="day-slider" id="adminDaySlider">
                    ...
                    <button class="day-slider-btn" data-day="3" onclick="switchAdminDay(this, 2)">MiÃ©</button>
                    ...
                    <button class="day-slider-btn" data-day="6" onclick="switchAdminDay(this, 5)">SÃ¡b</button>
                </div>
            </div>
            <div id="horariosList">
        ...
        <div class="section" id="section-asistencia">
            <div class="glass-card filter-bar">
                ...
                <div class="form-group">
                    <input type="text" id="filtroCedula" class="form-input" placeholder="Filtrar por cÃ©dula...">
                </div>
                <div class="form-group">
...
async function loadResumen() {
    ...
    const [c, h, u, a] = await Promise.all([
        ...
        api('horarios'),
        api('usuarios'),
        api('asistencia?mes=' + (new Date().getMonth() + 1) + '&anio=' + new Date().getFullYear())
    ]);
...
async function loadClases() {
    ...
    container.innerHTML = '<div class="cards-grid">' + result.data.map(c => `
        <div class="glass-card item-card">
            <h3>${c.nombre}</h3>
            <p>${c.descripcion || 'Sin descripciÃ³n'}</p>
...
async function createClase() {
    ...
    const result = await api('clases', 'POST', { nombre, descripcion: desc });
    if (result.success) {
...
async function deleteClase(id, nombre) {
    if (!confirm(`Â¿Eliminar la clase "${nombre}"? TambiÃ©n se eliminarÃ¡n sus horarios.`)) return;
    console.log('[ADMIN] deleteClase:', id, nombre);
    const result = await api('clases/' + id, 'DELETE');
    if (result.success) { showToast(result.message, 'success'); loadClases(); }
    else showToast(result.message, 'error');
}

// Horarios
const ADMIN_DAY_NAMES = {1:'Lunes',2:'Martes',3:'MiÃ©rcoles',4:'Jueves',5:'Viernes',6:'SÃ¡bado'};
...
async function loadHorarios() {
    ...
    const result = await api('horarios');
    if (!result.success || !result.data.length) {
        document.getElementById('horariosList').innerHTML = '<div class="empty-state"><span class="empty-icon"><i class="fas fa-calendar-alt"></i></span><p>No hay horarios configurados. AgregÃ¡ uno arriba.</p></div>';
        return;
    }
...
function renderAdminTimeline() {
    ...
    if (!daySchedules.length) {
        container.innerHTML = '<div class="empty-state" style="padding:40px 16px;"><span class="empty-icon"><i class="fas fa-calendar-alt"></i></span><p>No hay clases para este dÃ­a</p></div>';
        return;
    }
...
async function createHorario() {
    ...
    const result = await api('horarios', 'POST', { clase_id, dia_semana, hora_inicio, hora_fin });
    if (result.success) {
        showToast(result.message, 'success');
...
async function deleteHorario(id, nombre) {
    if (!confirm(`Â¿Eliminar "${nombre}" de este horario?`)) return;
    console.log('[ADMIN] deleteHorario:', id, nombre);
    const result = await api('horarios/' + id, 'DELETE');
...
async function loadAsistencia() {
    ...
    const result = await api(`asistencia?${params.toString()}`);
    ...
    container.innerHTML = `<div class="table-container"><table>
        <thead><tr><th>#</th><th>CÃ©dula</th><th>Nombre</th><th>Clase</th><th>Fecha</th><th>Hora</th></tr></thead>
...
async function loadReporte() {
    ...
    const result = await api(`asistencia/reporte-mensual?mes=${mes}&anio=${anio}`);
    ...
    container.innerHTML = `
        <div class="stats-grid">
            ...
            <div class="glass-card stat-card"><div class="stat-icon"><i class="fas fa-users"></i></div><div class="stat-value">${d.bailarines_unicos}</div><div class="stat-label">Bailarines Ãšnicos</div></div>
        </div>
        ...
        ${d.por_bailarin.length ? `<div class="glass-card" style="padding:24px;margin-bottom:20px;">
            <h3 style="margin-bottom:16px;">Asistencia por BailarÃ­n</h3>
            <div class="table-container"><table>
                <thead><tr><th>CÃ©dula</th><th>Nombre</th><th>Total</th></tr></thead>
        ...
        ${d.por_dia.length ? `<div class="glass-card" style="padding:24px;">
            <h3 style="margin-bottom:16px;">Asistencia por DÃ­a</h3>
            <div class="table-container"><table>
...
async function loadBailarines() {
    ...
    if (totalEl) totalEl.textContent = `${result.total} bailarÃ­n${result.total !== 1 ? 'es' : ''}`;

    container.innerHTML = `<div class="table-container"><table>
        <thead><tr><th>#</th><th>CÃ©dula</th><th>Nombre</th><th>Registrado</th><th>AcciÃ³n</th></tr></thead>
...
async function deleteBailarin(id, nombre) {
    if (!confirm(`Â¿Eliminar la cuenta de "${nombre}"?\n\nSe eliminarÃ¡ permanentemente.`)) return;
    console.log('[ADMIN] deleteBailarin:', id, nombre);
    const result = await api('usuarios/' + id, 'DELETE');
    if (result.success) {
        showToast(result.message, 'success');
        loadBailarines();
    } else {
