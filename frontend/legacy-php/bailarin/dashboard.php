...
async function loadHistory() {
    ...
    if (!result.success || result.data.length === 0) {
        container.innerHTML = '<div class="empty-state" style="padding:40px 16px;"><span class="empty-icon"><i class="fas fa-clipboard-list"></i></span><p>AÃºn no has asistido a clases</p><p style="font-size:0.85rem;color:var(--text-muted);margin-top:4px;">Escanea un QR en tu clase para registrar tu asistencia.</p></div>';
        return;
    }
    ...
    container.innerHTML = result.data.map(r => `
        <div class="glass-card" style="padding:14px 16px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;">
            <div>
                ...
                <div style="font-size:0.8rem;color:var(--text-muted);margin-top:2px;">${formatDate(r.fecha)} Â· ${formatTime(r.hora_registro)}</div>
            </div>
